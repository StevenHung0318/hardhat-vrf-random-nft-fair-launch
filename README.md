...
```
git clone https://github.com/StevenHung0318/hardhat-vrf-random-nft-fair-launch
cd hardhat-vrf-random-nft-fair-launch
yarn
```
...
```
yarn hardhat run script/mint.js
```

## Testing
(I didn't write test for now)
```
yarn hardhat test
```
...
We only deploy the `main` tags, since we need to add our `RandomNFT` contract as a consumer. 
